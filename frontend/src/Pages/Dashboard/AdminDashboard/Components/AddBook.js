      ...
      complete: async (result) => {
        try {
          for (const record of result.data) {
            const cleanedISBN = record.ISBN10 || record.ISBN13;

            if (!isbn10Regex.test(cleanedISBN) && !isbn13Regex.test(cleanedISBN)) {
              setSuccessMessage(`Invalid ISBN for record: ${JSON.stringify(record)}`);
              continue;
            }
            ...
            if (response.ok) {
              ...
              const count = record.BookCountAvailable;

              if (!number.test(count)) {
                setSuccessMessage(`Invalid Book Count for record: ${JSON.stringify(record)}`);
                continue;
              }
              ...
              const BookData = {
                ...
                noofpages: data.pageCount,
                coverpage: data.coverImage,
              ...
              try {
                ...
                console.error('Error adding Books from CSV:', err);
                setSuccessMessage(`Error adding Books from CSV: ${JSON.stringify(record)}`);
              }
            } else {
              console.error('Error fetching book details:', response.status);
              setSuccessMessage(`Error fetching book details for record: ${JSON.stringify(record)}`);
            }
          }
        } catch (error) {
          console.error('Error processing CSV record:', error);
          setSuccessMessage(`Error processing CSV record: ${error.message}`);
        }
      },
    });
  } catch (error) {
...
const toggleForm = () => {
  setIsCSVFormVisible(!isCSVFormVisible);
};
...
<div className="Top-Box">
      ...
      <div className="toggle-form">
        ...
        value={isCSVFormVisible ? 'Fill Manually' : 'Upload CSV'}
        onClick={toggleForm}
        />
      </div>
